/**
 * Created on 31 Jul 2022 by lonmee
 *
 */
...
class EventChannel extends ChannelSuper {
  post(event, ...msg) {
    console.log('[post to RN]', event, msg);
  ...
  trigger(event, ...msg) {
    this.listeners(event).every(value => value.apply(null, msg));
  }
...
class SystemChannel extends ChannelSuper {
  ...
  datadir() {
    if (this._cacheDataDir === null) {
      ...
      this._cacheDataDir = process.cwd() + '/ssb_data';
      // this._cacheDataDir = path.join(os.homedir(), 'metalife');
    }
